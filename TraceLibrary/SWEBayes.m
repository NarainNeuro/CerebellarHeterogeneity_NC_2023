function [e, likelihood] = SWEBayes(m,wm,xmin,xmax,varargin)
...
%           Note: DNA: Additional prior type: Bimodal
%                      requires mu1 mu2 and sig as parameters
%
%   e = ScalarBayesEstimator(...,'estimator',estimator_opts)
%       with
...
%                         e = sum(w.*X(1,:,:,:).*likelihood.*P,4)./sum(w.*likelihood.*P,4);
                    ...
                    case 'Gaussian'
                        ...
                        case 'Bimodal'
                        % Parameterize prior
                        % Assuming a mixture of Gaussians with diff means
                        % and same variance
                        mu1 = prior.mu1;
                        mu2 = prior.mu2;
                        sig = prior.sig;
                        ...
                        x = reshape(x,[1 1 1 l]);
                        X = repmat(x,[size(M,1) 1 size(M,3) 1]);
                        P = 1/sqrt(2*pi*sig^2) * exp( -(X(1,:,:,:)-mu1).^2/(2*sig^2))+ 1/sqrt(2*pi*sig^2) * exp( -(X(1,:,:,:)-mu2).^2/(2*sig^2));                  

                        % Generate estimate
                        w = reshape(w,[1 1 1 l]);
                        w = repmat(w,[1 1 size(m,1) 1]);
                        likelihood = 100.*( (1./sqrt(2*pi)/wm/X(1,:,:,:)).^N .* exp( -(sum((X-M).^2,1))./(2*wm.^2.*X(1,:,:,:).^2) ) );
                        %                likelihood = ( (1./sqrt(2*pi)/wm/X(1,:,:,:)).^N .* exp( -(mmx('mult',permute(X-M,[2 1 3 4]),X-M))./(2*wm.^2.*X(1,:,:,:).^2) ) );
                        e = nansum(w.*X(1,:,:,:).*likelihood.*P,4)./nansum(w.*likelihood.*P,4);
                        e = permute(e,[3 2 1]);
