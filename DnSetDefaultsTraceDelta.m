...
function [param] = DnSetDefaultsTraceDelta()
...
%% General Parameters
                    ...
                    param.genpars.time       = 1800;        % endtime time
...
%                   Gain function params:      default linear
                    ...
                    param.basis.slope        = 1e-5;                    % If linear
                    param.basis.const        = 0.001;                   % for both
                    param.basis.tau          = 2000;                    % If exponential, decay
                    param.basis.sigcomp      = 0.4;                     % By how much should the sigma decay (0 to 1)
                    param.basis.amp          = 0.6;                      
...
%% Parameters for the prior
                    ...
                    param.prior.uni(1,1:3)   = [200 0 0];%[529 0 0]; % In event of a single prior    
                    param.prior.uni(2,1:3)   = [500 0 0];%[1059 0 0];
                    
                    param.prior.gauss(1,1:3) = [350 0 0]; % If Gaussian prior is used   
                    param.prior.gauss(2,1:3) = [10 0 0];
                    param.prior.wm           = 0.1;
                    param.prior.range        = 201:500;
                    param.prior.stim         = param.genpars.timeline;
                    param.prior.unimu        = [200 350 500];
                    param.s3                 = '--------------------------------------------------';
